...
#from logiquefloue import tailleChemise, taillePantalon
from model_flou import tailleChemise, taillePantalon , tailleVeste
...
############################################################## CHOIX DE CHEMISES
        ...
        self.labelLongueurEpaule = customtkinter.CTkLabel(self.inputs_sncFrame, justify='left', text="Longueur Epaule (cm) : ", font=("Helvetica",14))
...
##############################################################
        ...
        # Création d'un conteneur pour le résumé du texte entré
        self.Ssummary_frame = customtkinter.CTkFrame(master=self.inputs_frthFrame, border_width=0, width=450)
        ...
        def summarizeCostume(self):
            ...
            if resultat is not None:
                self.summary_label.config(wraplength=1000, 
                                        justify='left', 
                                        text="Tour de taille :\t\t{} cm\ntoure poitrine :\t\t{} cm\ntaille dos :\t\t{} cm\nRésultat :\t\t\t{}".format(self.Stt_entry.get(), 
                                                                                                                                    self.Std_entry.get(),self.Stg_entry.get(), 
                                                                                                                                    resultat))
            else:
                self.summary_label.config(wraplength=1000, justify='left', text="Désolé, Nous n'avons pas de taille en accord avec vos mensurations !")
